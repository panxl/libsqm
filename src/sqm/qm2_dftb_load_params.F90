...
! In parallel all threads call this routine, but only commqmmm_master
! does any work here.
   ...
   if (.not. qmmm_mpi%commqmmm_master) return
   ...
   do i = 1,qmmm_struct%nquant_nlink 
      ...
      if (.not.do_repeat) then
         ...
         !=========================================
         ! There is NO check on whether an atom is supported any longer.
         ! The reason is that users can add any parameter sets they desire.
         !
...
subroutine qm2_dftb_check_slko_file(sk_file)
   ...
   if (.not.found) then
      ...
      write(6,*)" that the file was placed in the wrong directory,"
      write(6,*)" is using a different naming scheme, "
...
!! Called from deallocate_qmmm < qmmm_module.f >
   ...
   integer :: ier=0 ! Deallocation status
